(function() {
    ...
    // Advanced features
    const orbitTrails = []; // Array to store orbit trail lines
    let showTrails = true;
    let trailLength = 100;
    const blackHoles = [];
    ...
    function setupEventListeners() {
        ...
        const addBlackHoleBtn = document.getElementById("add-blackhole-btn");
        if (addBlackHoleBtn) {
            addBlackHoleBtn.addEventListener("click", () => {
                createBlackHole();
                showAIMessage("blackHoleCreated");
            });
        }
        ...
        if (showTrailsCheckbox) {
            showTrailsCheckbox.addEventListener("change", (e) => {
                ...
                orbitTrails.forEach(trail => trail.line.visible = showTrails);
            });
        }

        const showEnergyCheckbox = document.getElementById("show-energy");
        if (showEnergyCheckbox) {
            showEnergyCheckbox.addEventListener("change", (e) => {
                document.getElementById("energy-panel").classList.toggle("hidden", !e.target.checked);
            });
        }

        const spaceshipModeBtn = document.getElementById("spaceship-mode-btn");
        if (spaceshipModeBtn) {
            spaceshipModeBtn.addEventListener("click", () => {
                toggleSpaceshipMode();
                if (spaceshipMode) showAIMessage("spaceship");
            });
        }
        ...
        if (startMissionBtn) {
            startMissionBtn.addEventListener("click", () => {
                const missionType = Math.random() > 0.5 ? "stable" : "moon";
                startMission(missionType);
            });
        }

        const aiAssistantToggleBtn = document.getElementById("ai-assistant-toggle-btn");
        if (aiAssistantToggleBtn) {
            aiAssistantToggleBtn.addEventListener("click", () => {
    ...
    function animate() {
        ...
        if (spaceshipMode) updateSpaceship(deltaTime);
        if (document.getElementById("enable-collisions")?.checked) handleCollisions();
        if (document.getElementById("show-energy")?.checked) updateEnergyDisplay();
        blackHoles.forEach(bh => { bh.disk.rotation.z += bh.rotationSpeed * effectiveTimeScale; });
        if (showTrails) orbitTrails.forEach(trail => trail.update());
...
function createBlackHole(options = {}) {
    ...
    const position = options.position || new THREE.Vector3(
        (Math.random() - 0.5) * 200, (Math.random() - 0.5) * 50, (Math.random() - 0.5) * 200
    );
    const name = options.name || 'Black Hole ' + nextId++;
    const mass = options.mass || calculateMass('blackhole', size) * 100;
    
    const physicsBody = new PhysicsBody({
        id: Date.now(), mass: mass, position: position.clone(),
        velocity: new THREE.Vector3(0, 0, 0), radius: size, type: 'blackhole'
    });
    physicsEngine.addBody(physicsBody);
    ...
    const disk = new THREE.Mesh(
        new THREE.RingGeometry(size * 1.5, size * 3, 64),
        new THREE.MeshBasicMaterial({ color: 0xff4400, side: THREE.DoubleSide, transparent: true, opacity: 0.7 })
    );
    disk.rotation.x = Math.PI / 2;
    bhGroup.add(disk);
    
    const glow = new THREE.Mesh(
        new THREE.SphereGeometry(size * 4, 64, 64),
    ...
    const bhData = {
        id: physicsBody.id, type: 'blackhole', name: name,
        mesh: horizon, group: bhGroup, disk: disk, glow: glow,
        size: size, mass: mass, physicsBody: physicsBody, rotationSpeed: 0.01
    };
...
function createSpaceship() {
    ...
    const cockpit = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({ color: 0x4a90d9, roughness: 0.1, metalness: 0.9, transparent: true, opacity: 0.8 })
    );
    cockpit.position.set(0, 0.3, -0.5);
    shipGroup.add(cockpit);
    
    const glowMaterial = new THREE.MeshBasicMaterial({ color: 0x00d4ff, transparent: true, opacity: 0.8 });
    const leftGlow = new THREE.Mesh(new THREE.SphereGeometry(0.15, 8, 8), glowMaterial);
    leftGlow.position.set(-0.4, 0, 1.1);
    shipGroup.add(leftGlow);
    
    const rightGlow = new THREE.Mesh(new THREE.SphereGeometry(0.15, 8, 8), glowMaterial);
    ...
    spaceship = { mesh: shipGroup, velocity: new THREE.Vector3(0, 0, -1), leftGlow: leftGlow, rightGlow: rightGlow };
}
...
function updateSpaceship(deltaTime) {
    ...
    if (keys.w) spaceship.velocity.add(forward.multiplyScalar(20.0 * speedMultiplier * deltaTime));
    if (keys.s) spaceship.velocity.add(forward.multiplyScalar(-20.0 * speedMultiplier * deltaTime));
    if (keys.a) spaceship.mesh.rotateY(2.0 * deltaTime);
    if (keys.d) spaceship.mesh.rotateY(-2.0 * deltaTime);
    
    const maxSpeed = boostActive ? 150 : 50;
    if (spaceship.velocity.length() > maxSpeed) spaceship.velocity.normalize().multiplyScalar(maxSpeed);
...
function toggleSpaceshipMode() {
    ...
    if (spaceshipMode) {
        createSpaceship();
        document.getElementById('spaceship-hud').classList.remove('hidden');
        document.getElementById('spaceship-mode-btn').innerHTML = '<span class="btn-icon">🛑</span> Exit Spaceship';
        controls.enabled = false;
    } else {
        document.getElementById('spaceship-hud').classList.add('hidden');
        document.getElementById('spaceship-mode-btn').innerHTML = '<span class="btn-icon">🚀</span> Spaceship Mode';
...
function updateEnergyDisplay() {
    ...
    const kineticEl = document.querySelector('#energy-display .energy-item:nth-child(1) .energy-value');
    const potentialEl = document.querySelector('#energy-display .energy-item:nth-child(2) .energy-value');
    const totalEl = document.querySelector('#energy-display .energy-item:nth-child(3) .energy-value');
    if (kineticEl) kineticEl.textContent = energy.kinetic.toFixed(2);
    if (potentialEl) potentialEl.textContent = energy.potential.toFixed(2);
...
function handleCollisions() {
    ...
    for (const collision of collisions) {
        ...
        for (let i = planets.length - 1; i >= 0; i--) {
            if (planets[i].physicsBody === smaller) { smallerData = planets[i]; planets.splice(i, 1); break; }
        }
        if (!smallerData) {
            for (let i = moons.length - 1; i >= 0; i--) {
                if (moons[i].physicsBody === smaller) { smallerData = moons[i]; moons.splice(i, 1); break; }
            }
        }
        if (smallerData) {
            scene.remove(smallerData.mesh);
            if (smallerData.orbitLine) scene.remove(smallerData.orbitLine);
        }
        const growthFactor = Math.pow(smaller.mass / larger.mass + 1, 1/3);
        larger.radius *= growthFactor;
        const largerData = planets.find(p => p.physicsBody === larger) || moons.find(m => m.physicsBody === larger) || stars.find(s => s.physicsBody === larger);
        if (largerData) {
...
const aiMessages = {
    welcome: "Welcome to Universe Builder! I can help you create stable planetary systems. Try creating a star first!",
    starCreated: "Great! Stars are gravitational centers. Now add planets orbiting around it.",
    planetCreated: "Excellent! Planets orbit based on Kepler's laws. Closer planets orbit faster.",
    blackHoleCreated: "Warning: Black holes have extreme gravity! They can disrupt your system.",
    collision: "Collision occurred! The smaller object merged into the larger one.",
    spaceship: "Spaceship mode! Use WASD to move, SHIFT for boost."
};

function showAIMessage(key) {
    const messageEl = document.getElementById('ai-message');
    const panel = document.getElementById('ai-assistant-panel');
    if (messageEl && aiMessages[key]) {
        messageEl.textContent = aiMessages[key];
...
function startMission(type) {
    ...
    switch(type) {
        case 'stable':
            objective = { type: 'stable', description: 'Create a stable system with 1 star and 3+ planets', check: () => stars.length >= 1 && planets.length >= 3 };
            break;
        case 'moon':
            objective = { type: 'moon', description: 'Create a planet with at least 2 moons', check: () => moons.length >= 2 };
            break;
    }
    missionObjective = objective;
    if (panel) {
        panel.innerHTML = '<h4>🎯 Mission</h4><p>' + objective.description + '</p><div class="mission-progress">In Progress...</div>';
        panel.classList.remove('hidden');
    }
}
...
function checkMissionComplete() {
    ...
    if (missionObjective.check()) {
        ...
        const panel = document.getElementById('mission-panel');
        if (panel) {
            panel.innerHTML = '<h4>🎉 Mission Complete!</h4><p>' + missionObjective.description + '</p>';
        }
